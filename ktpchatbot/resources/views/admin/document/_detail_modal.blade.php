<div>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="py-2">
                    ...
                    @if ($request->status == 'ditolak' && !empty($request->alasan_ditolak))
                        <div class="mt-2 text-red-600 text-xs">
                            <strong>Alasan Penolakan:</strong> {{ $request->alasan_ditolak }}
                        </div>
                    @endif
                </td>
            </tr>
    ...
    <form id="form-update-status" action="{{ route('admin.document.updateStatus', $request->id) }}" method="POST"
        class="space-y-4">
        @csrf
        <div>
            ...
            <select id="status-select" name="status" class="w-full border rounded px-3 py-2">
                <option value="dalam proses verifikasi"
                    {{ $request->status == 'dalam proses verifikasi' ? 'selected' : '' }}>Dalam
                    Proses Verifikasi</option>
                <option value="dalam proses pencetakan"
                    {{ $request->status == 'dalam proses pencetakan' ? 'selected' : '' }}>Dalam
                    Proses Pencetakan</option>
                <option value="sudah tercetak" {{ $request->status == 'sudah tercetak' ? 'selected' : '' }}>Sudah
                    Tercetak</option>
                <option value="selesai pengambilan" {{ $request->status == 'selesai pengambilan' ? 'selected' : '' }}>
                    Selesai
                    Pengambilan</option>
        ...
        <div id="alasan-penolakan-container" style="display: {{ $request->status == 'ditolak' ? 'block' : 'none' }};">
            ...
            <select name="alasan_ditolak" class="w-full border rounded px-3 py-2"
                ...
                <option value="informasi tidak sesuai"
                    {{ $request->alasan_ditolak == 'informasi tidak sesuai' ? 'selected' : '' }}>Informasi tidak sesuai
                </option>
                <option value="kualitas gambar terlalu rendah"
                    {{ $request->alasan_ditolak == 'kualitas gambar terlalu rendah' ? 'selected' : '' }}>Kualitas
                    gambar terlalu
                    rendah</option>
                <option value="dokumen tidak terbaca jelas"
                    {{ $request->alasan_ditolak == 'dokumen tidak terbaca jelas' ? 'selected' : '' }}>Dokumen tidak
                    terbaca jelas
                </option>
                <option value="dokumen tidak valid"
                    {{ $request->alasan_ditolak == 'dokumen tidak valid' ? 'selected' : '' }}>
                    Dokumen tidak valid</option>
                <option value="persyaratan belum lengkap"
                    {{ $request->alasan_ditolak == 'persyaratan belum lengkap' ? 'selected' : '' }}>Persyaratan belum
                    lengkap</option>
                <option value="tidak memenuhi syarat"
                    {{ $request->alasan_ditolak == 'tidak memenuhi syarat' ? 'selected' : '' }}>
                    Tidak memenuhi syarat</option>
            </select>
        </div>

        <button type="submit" class="px-4 py-2 rounded transition bg-blue-600 text-white hover:bg-blue-700">
            Simpan Perubahan
        </button>

        <div id="status-update-msg" class="mt-2 text-sm"></div>
    </form>
</div>
